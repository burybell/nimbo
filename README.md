...
- Sandboxed Pre-request JavaScript for changing the current send and writing active-environment variables without blocking the UI
...
WebSocket, SSE, GraphQL, gRPC, collection runners, post-response and collection-level scripts, AI features, cloud sync, and accounts are not implemented yet. The interface intentionally does not expose unavailable features.
