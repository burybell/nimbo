...
后续全屏与集合回归：桌面全局搜索改为独立居中层，最大化后不再受左侧工作区提示或右侧环境/窗口按钮宽度影响；响应正文、Headers 和 Cookies 内容区统一占用 ResponseHeader 之后的剩余高度，JSON 最后一行和底部内边距可以完整滚动到；集合中的请求始终提供轻量编辑入口，重命名会同步集合节点、持久化请求以及同 ID 的已打开标签。
...
集合树层级回归：集合、文件夹和请求改为每层 32vp 的明确递进缩进，并限制极深目录的最大缩进，确保子请求的 Method 标签本身也落在父文件夹图标右侧，避免两级内容看起来位于同一条竖线上，同时保留窄侧栏中的名称可读空间。
